...
def sha224(message):
    ...
    return hash32(message, 224, hash_constants)
...
def sha256(message):
    ...
    return hash32(message, 256, hash_constants)

def sha384(message):
    '''Hashes message using SHA384.
    ...
    return hash64(message, 384, hash_constants)

def sha512(message):
    '''Hashes message using SHA512.
    ...
    return hash64(message, 512, hash_constants)

def sha512_224(message):
    '''Hashes message using SHA512 and truncates to 224 bits.
    ...
    return hash64(message, 224, hash_constants)

def sha512_256(message):
    '''Hashes message using SHA512 and truncates to 256 bits.
    ...
    return hash64(message, 256, hash_constants)


# Hash computation for 64-bit algorithms
# -----------------------------------------------------------------------------------------------------

def hash64(message, bit_length, hash_constants):
    ''' Helper function to perform hashing of data for 64-bit algorithms. 
    Returns the digest of message based on bit_length and hash_constants.
    ...
    bit_length : int 
        length of digest.
    hash_constants : list(int) 
        hash constants of given algorithm.
    ...
    rotr = lambda num, bits : (num >> bits) | (num << (64 - bits)) % 2**64
    ...
    # 4. Mutation Loop
    for block in range(0, len(blocks)):
        a, b, c, d, e, f, g, h = hash_constants

        for entry in range(0, 80):
            s0 = (rotr(e, 14)) ^ (rotr(e, 18)) ^ (rotr(e, 41))
            ch = (e & f) ^ (~e & g)
            temp1 = (h + s0 + ch + round_constants[entry] + blocks[block][entry]) % 2**64
        ...
        hash_constants[0] = (hash_constants[0] + a) % 2**64 
        hash_constants[1] = (hash_constants[1] + b) % 2**64 
        hash_constants[2] = (hash_constants[2] + c) % 2**64 
        hash_constants[3] = (hash_constants[3] + d) % 2**64 
        hash_constants[4] = (hash_constants[4] + e) % 2**64 
        hash_constants[5] = (hash_constants[5] + f) % 2**64
        hash_constants[6] = (hash_constants[6] + g) % 2**64
        hash_constants[7] = (hash_constants[7] + h) % 2**64 

    # 5. Digest Concatenation
    return ''.join([format(hash, '016x') for hash in hash_constants])[0 : bit_length//4]


# Hash computation for 32-bit algorithms
# -----------------------------------------------------------------------------------------------------

def hash32(message, bit_length, hash_constants):
    ''' Helper function to perform hashing of data for 32-bit algorithms. 
    Returns the digest of message based on bit_length and hash_constants.
    ...
    rotr = lambda num, bits : (num >> bits) | (num << (32 - bits)) % 2**32
    ...
    for block in range(0, len(blocks)):
        ...
        for entry in range(0, 64):
            ...
            ch = (e & f) ^ (~e & g)
            temp1 = (h + s0 + ch + round_constants[entry] + blocks[block][entry]) % 2**32
        ...
        hash_constants[5] = (hash_constants[5] + f) % 2**32
        hash_constants[6] = (hash_constants[6] + g) % 2**32
        hash_constants[7] = (hash_constants[7] + h) % 2**32

    # 5. Digest Concatenation
    return ''.join([format(hash, '08x') for hash in hash_constants])[0 : bit_length//4]
